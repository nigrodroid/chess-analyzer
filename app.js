...
function mateToWinPercent(mateIn) { return mateIn > 0 ? 100 : 0; }
function evalToWinPercent(evalData) {
  if (evalData.mate !== null && evalData.mate !== undefined) return mateToWinPercent(evalData.mate);
...
function classifyMove(cpLossForMover, isSacrifice, isBookMove, isTheTopEngineMove) {
...
function isPieceHangingAfterMove(fenAfterMove, toSquare) {
  try {
    const c = new Chess(fenAfterMove);
    return c.moves({ verbose: true }).some(m => m.to === toSquare && m.captured);
  } catch (e) { return false; }
}
function addBadgeToSquare(square, label) {
  $('.board-custom-badge').remove();
  const colors = { Brilliant:'#1baca6',Great:'#5c8bb0',Best:'#81b64c',Excellent:'#6fa84a',Good:'#4b643a',Book:'#a88865',Inaccuracy:'#f0c15c',Mistake:'#e67e22',Blunder:'#b33925' };
  const symbols = { Brilliant:'!!',Great:'!',Best:'✓',Excellent:'✓',Good:'✓',Book:'📖',Inaccuracy:'?!',Mistake:'?',Blunder:'??' };
  const color = colors[label] || '#888';
  const textColor = (label==='Inaccuracy'||label==='Book') ? '#000' : '#fff';
  $('.square-' + square).css('position','relative').append(
    `<div class="board-custom-badge" style="background-color:${color};color:${textColor};">${symbols[label]||''}</div>`
  );
}
function drawArrow(fromSquare, toSquare, color) {
  ...
  const center = sq => ({ x: files.indexOf(sq[0])*sqW+sqW/2, y: ranks.indexOf(sq[1])*sqH+sqH/2 });
...
function jumpToMove(index) {
  ...
  if (index === -1) { board.position('start'); return; }
...
function togglePlay() {
  if (isPlaying) {
    clearInterval(playInterval); $('#btnPlay').text('▶'); isPlaying = false;
  } else {
    if (currentMoveIndex >= analyzedMoves.length-1) currentMoveIndex = -1;
    $('#btnPlay').text('⏸'); isPlaying = true;
    playInterval = setInterval(() => {
      if (currentMoveIndex < analyzedMoves.length-1) jumpToMove(currentMoveIndex+1); else togglePlay();
    }, 1000);
  }
}
function updateNavButtonStates() {
  $('#btnFirst,#btnPrev').prop('disabled', currentMoveIndex <= -1);
  $('#btnNext,#btnLast').prop('disabled', currentMoveIndex >= analyzedMoves.length-1);
}

// ====== ALL BUTTON BINDINGS INSIDE document.ready — THIS IS THE FIX ======
$(document).ready(function() {
  ...
  $('#btnFirst').on('click', function() { if (!isAnalyzing) { jumpToMove(-1); updateNavButtonStates(); if (isPlaying) togglePlay(); } });
  $('#btnPrev').on('click', function() { if (!isAnalyzing) { jumpToMove(currentMoveIndex-1); updateNavButtonStates(); if (isPlaying) togglePlay(); } });
  $('#btnNext').on('click', function() { if (!isAnalyzing) { jumpToMove(currentMoveIndex+1); updateNavButtonStates(); if (isPlaying) togglePlay(); } });
  $('#btnLast').on('click', function() { if (!isAnalyzing) { jumpToMove(analyzedMoves.length-1); updateNavButtonStates(); if (isPlaying) togglePlay(); } });
  $('#btnPlay').on('click', function() { if (!isAnalyzing && analyzedMoves.length > 0) togglePlay(); });
  ...
  document.addEventListener('keydown', (e) => {
    ...
    else if (e.key==='ArrowUp')   { e.preventDefault(); jumpToMove(-1); updateNavButtonStates(); }
    else if (e.key==='ArrowDown') { e.preventDefault(); jumpToMove(analyzedMoves.length-1); updateNavButtonStates(); }
  });
});
...
function getEvalAndBestMove(fen, depth) {
  return new Promise((resolve) => {
    ...
    const onMessage = (event) => {
      ...
      if (cpMatch) { cp = sideToMoveIsWhite ? parseInt(cpMatch[1],10) : -parseInt(cpMatch[1],10); mate = null; }
      const mateMatch = line.match(/score mate (-?\d+)/);
      if (mateMatch) { mate = sideToMoveIsWhite ? parseInt(mateMatch[1],10) : -parseInt(mateMatch[1],10); cp = null; }
      if (line.startsWith('bestmove')) {
        const parts = line.split(' ');
        if (parts[1] && parts[1] !== '(none)') { bestFrom=parts[1].substring(0,2); bestTo=parts[1].substring(2,4); }
...
document.getElementById('analyzeBtn').addEventListener('click', async () => {
  ...
  game.reset();
  if (!game.load_pgn(pgn)) { resultsDiv.innerHTML = "<div style='padding:15px;color:#ff5555;'>Invalid PGN.</div>"; return; }
  const moves = game.history({ verbose: true });
  if (!moves.length) return;
  if (isPlaying) togglePlay();

  analyzedMoves = []; currentMoveIndex = -1;
  clearBadgesAndArrows();
  ...
  for (let i=0; i<moves.length; i++) {
    progressBar.style.width = Math.round(((i+1)/moves.length)*100)+'%';
    progressText.innerText = `Reviewing (${Math.round(((i+1)/moves.length)*100)}%)`;
    ...
    const pieceNowHanging = isPieceHangingAfterMove(testGame.fen(), moves[i].to);
    const isSacrifice = pieceNowHanging && cpLossForMover < 80 && pieceValue(moves[i].piece) >= 3;

    const classification = classifyMove(cpLossForMover, isSacrifice, isBookMove, isTopEngineMove);
    ...
    const evalDisplay = newEvalData.mate!==null ? `M${newEvalData.mate}` : ((cpAfter/100)>=0?'+':'')+(cpAfter/100).toFixed(2);
    const moveDiv = document.createElement('div');
    moveDiv.className='move-eval'; moveDiv.setAttribute('data-idx',i);
    moveDiv.innerHTML=`<span class="move-text">${prefix}${moves[i].san}</span><div class="badge-container"><span class="class-badge ${classification.cls}">${classification.label}</span><span class="eval-badge">${evalDisplay}</span></div>`;
  ...
  document.getElementById('whiteAccuracy').innerText = avg(accSamples.white).toFixed(1)+'%';
  document.getElementById('blackAccuracy').innerText = avg(accSamples.black).toFixed(1)+'%';

  const order = ['Brilliant','Great','Best','Excellent','Good','Book','Inaccuracy','Mistake','Blunder'];
  const colorMap = { Brilliant:'var(--cls-brilliant)',Great:'var(--cls-great)',Best:'var(--cls-best)',Excellent:'var(--cls-excellent)',Good:'var(--cls-good)',Book:'var(--cls-book)',Inaccuracy:'var(--cls-inaccuracy)',Mistake:'var(--cls-mistake)',Blunder:'var(--cls-blunder)' };
  document.getElementById('breakdown-body').innerHTML = order.map(label =>
    `<tr><td><span class="dot" style="background:${colorMap[label]}"></span>${label}</td><td class="count">${counts.white[label]}</td><td class="count">${counts.black[label]}</td></tr>`
  ).join('');
