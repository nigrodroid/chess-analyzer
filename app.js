...
async function loadStockfish() {
  ...
  resultsDiv.innerHTML = "<div style='color: #fff; font-size:12px; padding:10px;'>Spinning up Web Engine Worker...</div>";
  try {
    const response = await fetch('https://cdnjs.cloudflare.com/ajax/libs/stockfish.js/10.0.2/stockfish.js');
    if (!response.ok) throw new Error('Engine Script Fetch Failed');
    const code = await response.text();
    const blob = new Blob([code], { type: 'application/javascript' });
    stockfishWorker = new Worker(URL.createObjectURL(blob));
    resultsDiv.innerHTML = "";
    document.getElementById('coachText').innerText = "Engine active. Paste a game format and click analyze.";
  } catch (err) {
    document.getElementById('coachText').innerText = "Failed loading analytics module: " + err.message;
  }
}
loadStockfish();
...
function updateCoachCard(data) {
  ...
  const strings = {
    'Brilliant': "Spectacular tactical geometry! This sacrifice secures a winning initiative configuration.",
    'Best': "The optimal move choice. Keeps maximum positional pressure maintained on the objective vector.",
    'Excellent': "A very high-quality development choice that solidifies control of key board corridors.",
    'Good': "A safe, structural move that satisfies core safety paradigms without surrendering key files.",
    'Inaccuracy': "A step away from the absolute sharpest theoretical option. Handing over minor operational tempo.",
    'Mistake': "A processing oversight that surrenders strategic value. Tactical defensive parameters are weakened.",
    'Blunder': "A critical system oversight that exposes vital tactical structures. Severe advantage loss."
  };
  text.innerText = strings[data.classification.label] || "Move indexed perfectly.";
}
  ...
  return new Promise((resolve) => {
    ...
    const onMessage = (event) => {
      ...
      if (line.includes('score cp')) {
        ...
        if (cpMatch) cp = sideToMoveIsWhite ? parseInt(cpMatch[1], 10) : -parseInt(cpMatch[1], 10);
      } else if (line.includes('score mate')) {
        const mateMatch = line.match(/score mate (-?\d+)/);
        if (mateMatch) mate = sideToMoveIsWhite ? parseInt(mateMatch[1], 10) : -parseInt(mateMatch[1], 10);
      }
...
document.getElementById('analyzeBtn').addEventListener('click', async () => {
  ...
  for (let i = 0; i < moves.length; i++) {
    ...
    progressText.innerText = `Analyzing Move ${i+1}/${moves.length}`;
    ...
    const isSac = (!isWhiteMove && moves[i].captured && ['q','r','b','n'].includes(moves[i].captured));
    ...
    analyzedMoves.push({
      moveObj: moves[i], classification, bestMove: prevEvalData, evalCp: cpAfter, mate: newEvalData.mate
    });

    // Create Matrix Display Row Structure
